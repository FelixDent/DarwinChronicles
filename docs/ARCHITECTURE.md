...
Source files: `config.h/cpp` (CLI parsing, presets, `EnvParams` with plate fields, `tile_test` flag), `renderer.h/cpp` (tile rendering, camera), `telemetry.h/cpp` (overlays, stats, FPS), `main.cpp` (event loop + headless biome coloring + `--tile-test` mode). Shared from `sandboxes/weather/`: `terrain_gen.h/cpp` (generation pipeline — canonical in weather/), `tile_texture.h/cpp` (autotile template system — canonical in weather/). Local `terrain_gen.h/cpp` deleted; worldgen's `CMakeLists.txt` compiles the weather/ versions directly. Uses `sandbox` namespace.

**Tile-test mode** (`--tile-test`): renders 24×24-tile patches (16 px/tile) for 15 representative tile categories sampled from the generated terrain, saving one BMP per category. Used for visual verification of autotile output across different terrain types.
...
**Terrain rendering** (`renderer.h/cpp`, `tile_texture.h/cpp`): Two-level clipmap texture cache with discrete autotile templates. L0 macro (4 px/tile, whole world, baked once via `bake_terrain_cache()`) and L1 meso (16 px/tile, camera-region, regenerated on pan via `update_meso_cache()`). `TerrainCacheLevel` holds the SDL texture, world-space origin, and pixels-per-world-tile. `tile_texture.h/cpp` provides the complete autotile system: tile classification into `TileFamily` (PureTerrain/Coast/Elevation), `TransitionGrade` (Shallow/Medium/Steep), and `EdgePair` — forming discrete SNES/GBA-style tiles with Bezier curve boundaries. `TemplateAtlas` stores pre-generated 16×16 templates; 2 canonical edge-pair shapes × 3 grades × 3 variants, others derived via rotation LUT. `Mat` enum (12 material types from RockType/elevation/slope) drives per-tile base textures. 8 hypsometric elevation bands for warm→cool tinting. Edge blending between adjacent tiles. `eval_terrain_pixel()` and `render_terrain_region()` remain the core public API for clipmap baking. Dynamic wet/snow/standing-water tinting composited on top as semi-transparent quads. Mountain and hill glyphs removed. `dim_glyphs` parameter removed from `render_terrain()`. **Color ramps** (`renderer.cpp`): Temperature uses blue→white→red; Precipitation uses blue-only; Discharge uses teal→white; Stability uses diverging blue→white→red with gamma correction; Snow uses blue-tinted white. Wind arrows rendered in neutral gray with density reduction at low zoom. Telemetry panel has drop shadow/border, desaturated gold section headers, wider separator spacing.
...
Source files: `terrain_gen.h/cpp` (canonical terrain generation — worldgen sandbox now shares this file directly; includes tectonic mode and `EnvParams` with `plate_count`/`continental_ratio`), `weather.h/cpp` (static bake), `atmosphere.h/cpp` (atmosphere simulation), `dynamics.h/cpp` (terrain-level tick), `renderer.h/cpp` (tile rendering, clipmap cache, overlay dispatch), `tile_texture.h/cpp` (autotile template system — canonical here, shared by worldgen), `telemetry.h/cpp` (FPS, legend, `render_dynamic_legend` takes `AtmosphereStats`), `ui.h` (UI component library — see below), `main.cpp` (event loop with `TerrainPreset` table using `plate_count`/`continental_ratio`). Uses `sandbox` namespace.
