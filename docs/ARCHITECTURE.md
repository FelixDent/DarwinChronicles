...
**Terrain rendering** (`renderer.h/cpp`, `tile_texture.h/cpp`): Two-level clipmap texture cache replaces per-tile flat rectangles. L0 macro (4 px/tile, whole world, baked once via `bake_terrain_cache()`) and L1 meso (16 px/tile, camera-region, regenerated on pan via `update_meso_cache()`). `TerrainCacheLevel` holds the SDL texture, world-space origin, and pixels-per-world-tile. `tile_texture.h/cpp` provides `eval_terrain_pixel()` and `render_terrain_region()` — an LOD-gated per-pixel evaluator with hypsometric tint, geology-based rock color ramps, roughness/slope-based material texture, hillshade, coastal transitions, and scree on steep slopes. Dynamic wet/snow/standing-water tinting is composited on top as semi-transparent quads. Mountain and hill glyphs removed — procedural texture handles detail at all zoom levels. `dim_glyphs` parameter removed from `render_terrain()`. **Color ramps** (`renderer.cpp`): Temperature uses blue→white→red (perceptually uniform); Precipitation uses blue-only; Discharge uses teal→white; Stability uses diverging blue→white→red with gamma correction; Snow uses blue-tinted white. Wind arrows rendered in neutral gray with density reduction at low zoom. Telemetry panel has drop shadow/border, desaturated gold section headers, wider separator spacing.
...
Source files: `terrain_gen.h/cpp` (shared terrain — synced with worldgen, includes tectonic mode and `EnvParams` with `plate_count`/`continental_ratio`), `weather.h/cpp` (static bake), `atmosphere.h/cpp` (atmosphere simulation), `dynamics.h/cpp` (terrain-level tick), `renderer.h/cpp` (tile rendering, clipmap cache, overlay dispatch), `tile_texture.h/cpp` (LOD-gated procedural terrain pixel evaluator), `telemetry.h/cpp` (FPS, legend, `render_dynamic_legend` takes `AtmosphereStats`), `ui.h` (UI component library — see below), `main.cpp` (event loop with `TerrainPreset` table using `plate_count`/`continental_ratio`). Uses `sandbox` namespace.
