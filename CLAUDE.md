...
./build/tests/autotile_tests                  # Run autotile classification/rotation tests
```
...
- `sandboxes/weather/` — Atmospheric weather system layered on top of worldgen terrain. Three-phase architecture:
  ...
  - **Autotile template system** (`tile_texture.h/cpp`): discrete SNES/GBA-style tile classification replaces per-pixel continuous field sampling. Each tile is classified into one of three families: `PureTerrain` (uniform fill), `Coast` (land/water boundary), or `Elevation` (contour boundary). Transition tiles contain one continuous boundary line (Bezier curve from edge midpoint to edge midpoint) dividing the tile into two regions. `TransitionGrade` (Shallow/Medium/Steep) selects grade-specific treatment: beaches/rocky shores/cliffs for coast tiles; gradient/breakline/terrace for elevation tiles. **Template atlas**: two canonical `EdgePair` templates (N→S straight-through, N→E corner/arc) stored in `TemplateAtlas`; all 12 other edge combinations derived via a rotation LUT (`RotationEntry`: canonical + rotation_steps 0–3 + horizontal flip). `Mat` enum covers 12 material types (Water, Sand, SoilLoam, SoilDry, SoilClay, Gravel, Granite, Basalt, Limestone, Sandstone, Shale, Metamorphic); `classify_tile_mat()` maps `TerrainTile` to `Mat`. 3 pure-terrain variants per material plus 3 shape variants per (canonical, grade) combination. **Hypsometric elevation bands**: 8 warm→cool discrete levels for elevation tinting. **Edge blending**: adjacent-tile blending for seamless boundaries between tiles. `generate_template_atlas(atlas, seed)` must be called once before baking; `get_template_atlas()` returns the global instance. `TilePixels` struct (16×16 RGBA32, row-major) and `TileClassification` struct form the public API. Legacy API: `generate_tile_texture()` and `render_tile_patch()` retained for compatibility.
