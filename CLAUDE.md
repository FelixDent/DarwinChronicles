...
**Shared code between sandboxes MUST stay in sync.** When multiple sandboxes contain copies of the same system (e.g., `terrain_gen.cpp` exists in both `worldgen/` and `weather/`), changes to the shared logic (generation pipeline, erosion, uplift, geology) **must be applied to all copies**. The canonical version is whichever sandbox is most actively developed — currently `weather/` for terrain generation. After modifying shared code, propagate changes to all other copies and verify they build. If interfaces have diverged (different header fields), apply the algorithmic changes manually rather than blind-copying files. Long-term, shared code should be extracted into a common module to eliminate drift.
...
- `sandboxes/weather/` — Atmospheric weather system layered on top of worldgen terrain. Three-phase architecture:
  ...
  - **Terrain glyphs**: alpha-blended at 40–70 alpha when any overlay is active (`dim_glyphs` path in `render_terrain`), preventing overlay colors from being washed out by terrain tinting.
