...
- **`worldgen/`** — Tectonic terrain generation (12-stage pipeline: plates → Voronoi → tectonic fields → erosion → coastal remodeling → geology). Modes: `--headless`, `--tile-test`. 9 overlays, 6 presets. Shares `terrain_gen.h/cpp` and `tile_texture.h/cpp` from `weather/`.
