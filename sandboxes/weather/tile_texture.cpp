...
TilePixels generate_tile_texture(const Terrain& terrain, int tile_x, int tile_y, uint32_t seed,
    ...
    for (int py = 0; py < SZ; ++py) {
        for (int px = 0; px < SZ; ++px) {
            // World-space pixel position for noise (continuous across tiles)
            float wx = static_cast<float>(tile_x * SZ + px);
            float wy = static_cast<float>(tile_y * SZ + py);

            // Sub-tile position in tile-coordinate space
            float tile_fx = static_cast<float>(tile_x) + static_cast<float>(px) * INV_SZ;
            float tile_fy = static_cast<float>(tile_y) + static_cast<float>(py) * INV_SZ;
            ...
            float detail_gate = elev_factor * density_factor * tect_boost;
            ...
            // FINE: craggy peaks (isotropic)
            float r3_raw = ns->ridge_fine.GetNoise(dwx * 0.7f, dwy * 0.7f);
            float ridge3 = std::max(0.0f, 1.0f - std::abs(r3_raw) * 2.0f);

            // EXTRA FINE: tributary-scale fractal detail (1/3 of main ridge wavelength)
            // Adds broken-up, branchy structure that simple 3-octave ridged noise misses
            float r4_raw = ns->ridge_fine.GetNoise(dwx * 1.8f + 500.0f, dwy * 1.8f + 500.0f);
            float ridge4 = std::max(0.0f, 1.0f - std::abs(r4_raw) * 2.0f);

            float detail_raw = ridge1 * 0.42f + ridge2 * 0.28f + ridge3 * 0.18f + ridge4 * 0.12f;
            ...
            {
                constexpr float grad_eps = 0.7f;  // wide enough to smooth tile-boundary noise
                auto detailed_elev = [&](float fx, float fy) -> float {
                    ...
                    float nwx = fx * static_cast<float>(SZ);
                    float nwy = fy * static_cast<float>(SZ);
                    // Same gating as main elevation: continuous ramp from coast to peaks
                    ...
                    float dgn = std::clamp(ef * dfn * tbn, 0.0f, 1.0f);
                    float amp = dgn * 0.26f;
                    float ws = 70.0f;
                    ...
                    float nrd3 = std::max(0.0f, 1.0f - std::abs(nr3) * 2.0f);
                    float nr4 = ns->ridge_fine.GetNoise(ndwx*1.8f+500.0f, ndwy*1.8f+500.0f);
                    float nrd4 = std::max(0.0f, 1.0f - std::abs(nr4) * 2.0f);
                    float dh_raw = nrd1 * 0.42f + nrd2 * 0.28f + nrd3 * 0.18f + nrd4 * 0.12f;
            ...
            if (coastal_blend > 0.6f) {
                ...
                // --- 7. Texture detail layer ---
                // Lowlands get visible undulation (rolling hills, eroded plains).
                // Mountains get subtle grain from weathering.
                ...
                // Lowland undulation: multi-scale texture gives rolling-hill feel
                float lowland_tex = (1.0f - mtn_factor) *
                    (med_tex * 0.06f + coarse_tex * 0.04f + fine * 0.02f);
                ...
                float shade = hillshade(px_slope, px_aspect, 20.0f);
            ...
            out.set(px, py, static_cast<uint8_t>(color.r), static_cast<uint8_t>(color.g),
...
// ═══════════════════════════════════════════════════════════════════════════
// Render a rectangular patch of tiles to a pixel buffer
// ═══════════════════════════════════════════════════════════════════════════
...
void render_tile_patch(const Terrain& terrain, int patch_x, int patch_y, int patch_w, int patch_h,
    ...
    for (int ty = 0; ty < patch_h; ++ty) {
        for (int tx = 0; tx < patch_w; ++tx) {
            ...
            TilePixels pixels = generate_tile_texture(terrain, wx, wy, seed, water_level);

            for (int py = 0; py < SZ; ++py) {
                int out_y = ty * SZ + py;
                for (int px = 0; px < SZ; ++px) {
                    int out_x = tx * SZ + px;
                    out_pixels[out_y * out_stride + out_x] = pixels.pixels[py * SZ + px];
                }
            }
        }
    }
