...
void Renderer::render_terrain(const Terrain& world, const Camera& cam, int win_w, int win_h,
    ...
    for (int ty = min_ty; ty <= max_ty; ++ty) {
        for (int tx = min_tx; tx <= max_tx; ++tx) {
            ...
            if (dyn && !tile.is_ocean && !tile.is_lake) {
                const auto& dt =
                ...
                color[1] = static_cast<uint8_t>(std::clamp(
                    static_cast<float>(color[1]) * (1.0f + wet * 0.08f), 0.0f, 255.0f));
                ...
                if (dt.snow_depth > 0.01f) {
                    ...
                    color[0] = static_cast<uint8_t>(
                        static_cast<float>(color[0]) * (1.0f - snow_t) + 240.0f * snow_t);
                    color[1] = static_cast<uint8_t>(
                        static_cast<float>(color[1]) * (1.0f - snow_t) + 240.0f * snow_t);
                    color[2] = static_cast<uint8_t>(
                        static_cast<float>(color[2]) * (1.0f - snow_t) + 255.0f * snow_t);
                }
            ...
            if (!tile.is_ocean && !tile.is_lake && dst.w >= 4) {
                ...
                if (rough >= 0.45f) {
                    ...
                    if (rough >= 0.65f) {
                        // Snow-capped peaks use dynamic snow if available
                        bool has_snow = dyn && dyn->tile_at(static_cast<uint32_t>(tx),
                                                             static_cast<uint32_t>(ty))
                                                        .snow_depth > 0.05f;
                        if (has_snow || rough >= 0.65f) {
        ...
        for (int tx = min_tx; tx <= max_tx; ++tx) {
            ...
            switch (mode) {
                ...
                case OverlayMode::Evaporation: {
                    ...
                    if (dyn && !dyn->paused) {
                        const auto& dt = dyn->tile_at(static_cast<uint32_t>(tx),
                                                      static_cast<uint32_t>(ty));
                ...
                case OverlayMode::RainShadow: {
                    ...
                    if (atmo && dyn && !dyn->paused) {
                        ...
                        for (int step = 1; step <= 12; ++step) {
                            ...
                            if (ux < 0 || ux >= static_cast<int>(world.width) ||
                                uy < 0 || uy >= static_cast<int>(world.height))
                                break;
                            const auto& uptt = world.tile_at(
                                static_cast<uint32_t>(ux), static_cast<uint32_t>(uy));
...
void Renderer::render_wind_arrows(const ClimateData& climate, const Camera& cam, int win_w,
    ...
    for (int ty = min_ty; ty <= max_ty; ty += step) {
        for (int tx = min_tx; tx <= max_tx; tx += step) {
            ...
            float arrow_len = static_cast<float>(dst.w) * 0.8f * std::min(speed, 1.5f);

            float ex = cx + dx * arrow_len;
            float ey = cy + dy * arrow_len;

            auto alpha = static_cast<uint8_t>(std::clamp(speed * 200.0f, 80.0f, 255.0f));
            ...
            // Bright foreground: yellow-white so it contrasts on both light and dark
            draw_arrow(cx, cy, ex, ey, dx, dy, arrow_len, 255, 240, 100, alpha);
        }
    }
}
