...
- [x] Procedural terrain generation — 12-stage tectonic pipeline with coastal geomorphology: province/craton mask, continuous continental field, spines with along-strike fragmentation, along-strike mountain belt segmentation, meso-scale ridge noise, basin depressions, terrain variation noise, stream-power erosion (40 iterations), valley-floor smoothing (5b), valley drowning for rias/estuaries/fjords (5d), inland elevation bias, noise-warped coastal plain enforcement (6b), exposure-based SDF coastal remodeling with wave fetch + anisotropic displacement (7), depositional features — deltas + barrier islands (7b), clustered skerries (7c), sub-bay inlet carving (7d), basin flood-fill lakes, debris islet removal (9b), morphological narrow feature cleanup (9c), SDF curvature flow coastline smoothing (10); geology-first architecture: `compute_geology()` assigns RockType + SoilTexture + hydraulic parameters (ksat, field capacity, wilting point) per tile based on tectonic context; legacy noise mode; shared pipeline: ocean flood-fill, slope, aspect, EDT (dist_ocean), roughness, geology; headless BMP with climate-driven biome coloring and tectonics-aware bathymetry (`sandboxes/worldgen/`)
- [x] Atmospheric weather simulation — static bake (temperature, wind, precipitation, storminess, rain shadow) + atmosphere simulation (coarse-grid T/q/wind/cloud/precip with two-layer quasi-geostrophic dynamical core: stochastic baroclinic eddy parameterization, SOR PV inversion for streamfunctions, PGF surface winds, Ekman drag; latent heat, solar forcing, 10 physical invariants enforced) + dynamic terrain tick (rain/snow, snowmelt, geology-based infiltration with ksat/field_capacity, groundwater reservoir with baseflow, WSE multi-neighbor runoff routing, basin spillway storage for persistent lakes, two-reservoir D8 discharge with quickflow/baseflow separation, aridity tracking); discrete autotile terrain rendering (SNES/GBA-style template atlas with Bezier boundaries, 12 material types, 3 coast/elevation grades, rotation LUT); 20 overlay modes; speed keys `,`/`.` (comma/period); camera fits world on startup; headless `--screenshots` mode for GPT UI review (`sandboxes/weather/`)
